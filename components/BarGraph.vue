...
export default {
  ...
  methods:{
    render() {
      ...
      Bottom.call(d3.axisBottom(x).tickSize(0));
      ...
      this.update = (idx)=>{
          ...
          x
          .domain([0,d3.max(data.map(a=>parseFloat(a[1])))])
          .range([0,this.width - cap])

          y.domain(data.map(a=>a[0]))
          Bottom.call(d3.axisBottom(y).tickSize(0));
          bars
          .data(data)
          ...
          .data(data)
          .transition().duration(300).ease(d3.easeQuadOut)
          .attr("y",d=>x(d[1]) + ywidth + gap)
          .attr("x",d=>y(d[0])+y.bandwidth()*.5)
      ...
      this.comp.onIntersection(()=>{
        ...
        txts_
        .transition().duration(300).ease(d3.easeQuadOut)
        .delay((d,i)=>500 + 100*i).attr("y",d=>x(d[1]) + ywidth + gap);
      });
